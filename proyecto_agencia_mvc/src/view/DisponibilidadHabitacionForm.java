...
import dao.disponibilidadhabitaciondao;

public class DisponibilidadHabitacionForm extends JFrame {

    private final JComboBox<hotel> cbHotel = new JComboBox<>();
    private final JComboBox<tipohabitacion> cbTipoHab = new JComboBox<>();
    private final JTextField txtPlazas = new JTextField(8);
    ...
    private final DefaultTableModel modelo = new DefaultTableModel(
            new Object[]{"ID", "Hotel", "Tipo Habitación", "Plazas disponibles"}, 0
    ) {
        @Override public boolean isCellEditable(int r, int c) { return false; }
        @Override public Class<?> getColumnClass(int c) {
            return c == 0 ? Long.class : (c == 3 ? Integer.class : String.class);
        }
    };

    private TableRowSorter<DefaultTableModel> sorter;
    private Long seleccionadoId = null;
    private List<disponibilidadhabitacion> listaDisp;

    public DisponibilidadHabitacionForm() {
        setTitle("Disponibilidad por Tipo de Habitación");
        ...
        addRow(form, gc, y++, "Plazas disponibles:", txtPlazas);
    ...
    private void addRow(JPanel panel, GridBagConstraints gc, int y, String label, JComponent field) {
        ...
        if (field instanceof JTextField tf) styleField(tf);
        panel.add(field, gc);
    }
    ...
    private void configurarEventos() {
        tabla.getSelectionModel().addListSelectionListener(e -> {
            ...
            if (viewRow >= 0 && listaDisp != null) {
                int modelRow = tabla.convertRowIndexToModel(viewRow);
                disponibilidadhabitacion d = listaDisp.get(modelRow);

                seleccionadoId = d.getid_disponibilidad();
                seleccionarHotel(d.getid_hotel());
                seleccionarTipoHab(d.getid_tipo_hab());
                txtPlazas.setText(String.valueOf(d.getplazas_disponibles()));
    ...
    private void cargarTabla() {
        try {
            ...
            disponibilidadhabitaciondao dao = new disponibilidadhabitaciondao();
            listaDisp = dao.listar();

            for (disponibilidadhabitacion d : listaDisp) {
                modelo.addRow(new Object[]{
                        d.getid_disponibilidad(),
                        d.getNombreHotel(),
                        d.getTipoHabitacion(),
                        d.getplazas_disponibles()
                });
            }
            limpiarForm();
        } catch (Exception ex) {
    ...
    private void guardar() {
        ...
        if (plazasTxt.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Ingrese las plazas disponibles.");
            txtPlazas.requestFocus();
            return;
        }
        ...
        int plazas;
        try {
            plazas = Integer.parseInt(plazasTxt);
            if (plazas < 0) throw new NumberFormatException();
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(this, "Número de plazas inválido.");
            txtPlazas.requestFocus();
            return;
        }
        ...
        try {
            ...
            d.setid_hotel(h.getid_hotel());
            d.setid_tipo_hab(t.getid_tipo_hab());
            d.setplazas_disponibles(plazas);

            if (seleccionadoId == null) {
                dao.insert(d);
                JOptionPane.showMessageDialog(this, "Disponibilidad creada con éxito.");
            } else {
                d.setid_disponibilidad(seleccionadoId);
                dao.update(d);
    ...
    private void eliminar() {
        ...
        int r = JOptionPane.showConfirmDialog(this,
                "¿Deseas eliminar la disponibilidad seleccionada?",
                "Confirmar", JOptionPane.YES_NO_OPTION);

        if (r != JOptionPane.YES_OPTION) return;

        try {
            new disponibilidadhabitaciondao().delete(seleccionadoId);
            JOptionPane.showMessageDialog(this, "Disponibilidad eliminada.");
            cargarTabla();
        } catch (Exception ex) {
